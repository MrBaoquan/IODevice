...
> 本文档是 [动感平台编辑系统\_架构设计.md](动感平台编辑系统_架构设计.md) (v10) 的落地执行文件。  
> **起始日期：** 2026-03-09  
...
| 节点   | 截止日期  | 状态      | 说明                                  |
| ------ | --------- | --------- | ------------------------------------- |
| 节点一 | 2026/3/31 | ✅ 完成   | 架构设计 + 编辑器原型 + 播放引擎      |
| 节点二 | 2026/4/30 | 🔧 进行中 | 多 Slot 架构 + TCP 播控 + 播放器核心  |
| 节点三 | 2026/5/15 | 🔧 进行中 | C++ MotionPlayer + Wrapper + 编排优化 |
| 节点四 | 2026/5/31 | ⏸️ 未开始 | 效果预设 + 性能优化 + 全系统联调      |
| 节点五 | 2026/6/30 | ⏸️ 未开始 | 授权系统 + 打包 + 文档 + 验收         |

### 1.2 Phase 完成度

| Phase              | 内容                                           | 状态        |
| ------------------ | ---------------------------------------------- | ----------- |
| Phase 1            | 数据模型 + 播放引擎 + 插值                     | ✅          |
| Phase 2            | 时间轴基础 UI (12 Sprint)                      | ✅          |
| Phase 2.5          | 编辑器增强 (Undo/多选/快捷键)                  | ✅          |
| Phase 3 Sprint 3.1 | 编辑器内预览播放                               | ✅          |
| Phase 3 Sprint 3.2 | 导出集成                                       | 🔧 部分完成 |
| Phase 3 Sprint 3.3 | .mtn AES-256-GCM 加密                          | ❌ 未实现   |
| Phase 4            | 曲线编辑器                                     | ✅          |
| Phase 4.5-4.12     | 优化与体验提升                                 | ✅          |
| Phase 4.13         | 架构改进 — 短期                                | ✅          |
| Phase 4.14         | 架构改进 — 中期                                | ✅          |
| Phase 4.15         | 架构改进 — 长期 (DI/接口/测试)                 | ✅          |
| Phase 5            | 媒体同步 (视频+音频波形)                       | ✅          |
| C++ MotionPlayer   | 多 Slot 管理器 + 插值 + 混合 + 安全 + 设备派发 | ✅          |
| C Wrapper          | 18 个 Slot API 函数                            | ✅          |
| C# Wrapper         | IOToolkit.MotionPlayer 静态类                  | ✅          |
| Phase 6            | 效果预设系统                                   | ❌ 未开始   |
| TCP MotionServer   | 网络播控服务                                   | ❌ 未开始   |
| LicHper 授权       | Basic/Pro 运行时分级                           | ❌ 未开始   |

---

## 二、已完成工作

### 2.1 C++ 核心层 (IODevice 项目)

| 文件                              | 内容                                                                               | 代码量  |
| --------------------------------- | ---------------------------------------------------------------------------------- | ------- |
| `Source/Public/MotionPlayer.h`    | Slot API 头文件 (MotionState/MotionEvent/MixPolicy 枚举 + MotionPlayer 单例)       | 71 行   |
| `Source/Private/MotionPlayer.cpp` | 完整实现 (内部数据结构/插值/SafetyGuard/ChannelMixer/JSON解析/Tick流水线/设备派发) | ~750 行 |
| `Source/Vendor/nlohmann/json.hpp` | nlohmann/json v3.11.3 header-only                                                  | 依赖    |
| `IODeviceController.cpp` L99      | `MotionPlayer::Instance().Tick(deltaSeconds)` 集成点                               | 1 行    |

### 2.2 C/C# Wrapper 层

| 文件                                                        | 内容                                    |
| ----------------------------------------------------------- | --------------------------------------- |
| `IODevice_C_Wrapper/IODevice_CWrapper.h`                    | 18 个 `IOCAPI __stdcall` 函数声明       |
| `IODevice_C_Wrapper/IODevice_CWrapper.cpp`                  | 18 个函数实现，直接转发到 MotionPlayer  |
| `IODevice_CSharp_Wrapper/IOToolkit/Core/IONativeWrapper.cs` | 18 个 DllImport 声明                    |
| `IODevice_CSharp_Wrapper/IOToolkit/Facades/MotionPlayer.cs` | 静态门面类 + MotionState/MixPolicy 枚举 |

### 2.3 IOStudio 编辑器

| 类别        | 文件数         | 核心组件                                                                                                  |
| ----------- | -------------- | --------------------------------------------------------------------------------------------------------- |
| 数据模型    | 13             | MotionTimeline, MotionTrack, MotionClip, MotionKeyframe, TimelineMarker, TrackGroup 等                    |
| 播放引擎    | 1              | MotionPlaybackEngine — 状态机 + Tick 驱动                                                                 |
| 插值引擎    | 1              | InterpolationEngine — Linear/Bezier/Step/EaseInOut, 二分查找                                              |
| 安全保护    | 1              | SafetyGuard — Clamp + 速度限制 + SmoothStep 回中                                                          |
| 设备分发    | 1              | DeviceDispatcher — OAction/OAxis → IODevice.SetDO()                                                       |
| 文件读写    | 1              | MotionFileReader — JSON 读写 (System.Text.Json)                                                           |
| 导出服务    | 1              | MotionExportService — .motion JSON + CSV + CompactJson                                                    |
| Undo/Redo   | 1              | UndoRedoService — Command 模式, 100 级栈                                                                  |
| 快捷键      | 1              | KeyboardShortcutService — Ctrl 组合键 + 上下文感知                                                        |
| 曲线预设    | 1              | CurvePresetService — 10 种内置预设                                                                        |
| 标记服务    | 1              | MarkerService — CRUD + Undo + 导航                                                                        |
| 视频播放    | 1              | LibVlcVideoPlayerService — LibVLC 懒加载 + HWND 绑定                                                      |
| 音频波形    | 1              | AudioWaveformExtractor — PCM 捕获 + 渐进式降采样                                                          |
| 节拍检测    | 1              | BeatDetector — 能量阈值 + 滑动窗口                                                                        |
| 接口层      | 10             | IPlaybackEngine, IUndoRedoService 等全部定义                                                              |
| ViewModel   | 16 (7 partial) | TimelineEditorViewModel (6 partial), TrackVM, ClipVM, CurveEditorVM 等                                    |
| 自定义控件  | 13             | CurveEditorControl, TimeRulerControl, TrackClipControl, PlayheadOverlay, WaveformControl, VlcVideoHost 等 |
| 对话框/窗口 | 4              | TimelineEditorWindow, AddTrackDialog, ExportDialog, GroupDialog                                           |
| 单元测试    | 3              | UndoRedoServiceTests, MotionModelSerializationTests, KeyframePropertyViewModelTests                       |

### 2.4 构建状态

| 构建目标                    | 状态        |
| --------------------------- | ----------- |
| IODevice.dll (C++20, Win64) | ✅ 通过     |
| IODevice_C_Wrapper.dll      | ✅ 通过     |
| IODevice_CSharp_Wrapper.dll | ✅ 通过     |
| IOStudio (Avalonia)         | ✅ 通过     |
| 单元测试 (19 个)            | ✅ 全部通过 |
...
### 3.1 [P0] .motion JSON 格式兼容性

C# 编辑器写出的 JSON 与 C++ 运行时期望的 JSON **字段名不一致**：

| 字段         | C# 编辑器写出 (修复前)       | C++ 运行时期望        | 修复后 C# 输出        | 状态      |
| ------------ | ---------------------------- | --------------------- | --------------------- | --------- |
| 设备名       | `device_name`                | `device`              | `device`              | ✅ 已修复 |
| OAction 名   | `oaction_name`               | `oaction`             | `oaction`             | ✅ 已修复 |
| 贝塞尔控制点 | `tangent_in` / `tangent_out` | `cp1x/cp1y/cp2x/cp2y` | `cp1x/cp1y/cp2x/cp2y` | ✅ 已修复 |
| 插值类型     | `ease_in_out`                | `ease`                | 双向兼容              | ✅ 已修复 |

**修复记录：**
- 修改 `MotionTrack.cs` 的 `[JsonPropertyName]`: `"device_name"` → `"device"`, `"oaction_name"` → `"oaction"`
- 修改 `MotionKeyframe.cs` 的 `[JsonPropertyName]`: `"tangent_in"` → `"cp1y"`, `"tangent_out"` → `"cp2y"`
- 新增 `Cp1x` / `Cp2x` 属性 (对应 C++ `cp1x` / `cp2x`)
- `InterpolationEngine.cs` 增加 `"ease"` 兼容识别

> **注意：** 贝塞尔值语义差异 (C# 使用斜率参数化, C++ 使用绝对控制点位置) 尚未对齐，
> 曲线在 C# 编辑器预览与 C++ 运行时之间可能存在微小差异。此问题将在 "C# 播放路径统一至 C++ Wrapper" 时一并解决。

### 3.2 [P1] 编辑器播放路径未使用 C++ 核心

IOStudio 编辑器使用独立的 C# MotionPlaybackEngine 进行播放预览，不通过 C++ MotionPlayer。两套引擎的插值/安全保护/派发逻辑独立实现，可能产生行为差异。

**修复方案：** 参见架构设计文档 § 八 推荐演进方向。

**预估工时：** ~16h (非紧急，可分阶段推进)

### 3.3 [P2] Avalonia NuGet 版本不一致 — ✅ 已修复

`Avalonia.Controls.ItemsRepeater` 11.1.5 已移除 (Avalonia 11.2+ 已将 ItemsRepeater 合并至核心包)。所有 Avalonia 包统一为 11.3.10。

### 3.4 [P2] net6.0 已停止支持 — ✅ 已修复

IOStudio 和 IOStudio.Tests 已从 net6.0 升级至 net8.0 (LTS)。
- `Microsoft.Extensions.DependencyInjection` 6.0.1 → 8.0.1
- 19 个单元测试全部通过
...
| 优先级 | 任务                                     | 预估工时 | 说明                                                  |
| ------ | ---------------------------------------- | -------- | ----------------------------------------------------- |
| ~~P0~~ | ~~.motion JSON 格式兼容性修复~~          | ~~2-4h~~ | ✅ 已完成 — 字段名已对齐 C++                          |
| **P0** | **.mtn AES-256-GCM 加密导出 + 解密加载** | 6h       | MotionCryptoService: C# 加密导出 + C++ 解密加载       |
| **P0** | **TCP MotionServer**                     | 20h      | Slot 寻址 JSON 协议, TCP:9600, 状态推送               |
| **P0** | **影院视频播放器核心**                   | 16h      | 独立播放窗口 + 控制接口                               |
| **P1** | 端到端联调 (编辑→导出→TCP加载→设备播放)  | 4h       | 全链路验证                                            |
| **P1** | 编辑器播放路径统一 (C# → C++ Wrapper)    | 16h      | 可选：消除双引擎问题                                  |
| **P1** | 核心引擎单元测试补齐                     | 8h       | PlaybackEngine/Interpolation/SafetyGuard/ChannelMixer |
| **P2** | Avalonia 版本统一 + net6.0→net8.0 升级   | ~~4h~~   | ✅ 已完成 — ItemsRepeater 移除 + net8.0 |

### 4.2 中期 — 节点四

| 优先级 | 任务                           | 预估工时 | 说明                                     |
| ------ | ------------------------------ | -------- | ---------------------------------------- |
| **P1** | 效果预设系统                   | 16h      | 预设库面板 + 拖拽到轨道 + 自动生成关键帧 |
| **P1** | 性能优化 (>1000 关键帧 >30fps) | 3h       | 虚拟化渲染 / LOD                         |
| **P2** | 全系统端到端联调               | 8h       | IOStudio + C++ MotionPlayer            |
| **P2** | Bug 回归修复 + 全功能测试      | 4h       | —                                        |

### 4.3 后期 — 节点五

| 优先级 | 任务                                    | 预估工时 | 说明 |
| ------ | --------------------------------------- | -------- | ---- |
| **P0** | LicHper 授权集成 (Basic/Pro 运行时分级) | 12h      | —    |
| **P1** | IOStudio.exe 安装包打包                 | 8h       | —    |
| **P1** | 用户操作手册 + TCP 播控 API 文档        | 12h      | —    |
| **P2** | 全流程验收测试                          | 8h       | —    |
| **P2** | Buffer / 紧急 Bug 修复                  | 8h       | —    |
...
| ----- | -------- | --------------------- |
| 一 ✅ | 3/31     | 架构+引擎+编辑器原型  |
| 二    | 4/30     | 多Slot架构+TCP+播放器 |
| 三    | 5/15     | C++MotionPlayer+优化  |
| 四    | 5/31     | 效果预设+联调         |
| 五    | 6/30     | 授权+打包+验收        |
...
| 任务                                 | 工时 | 计划周期 | 前置依赖                    |
| ------------------------------------ | ---- | -------- | --------------------------- |
| ~~.motion JSON 格式兼容性修复~~      | ~~2-4h~~ | ~~立即~~ | ✅ 已完成                   |
| .mtn AES-256-GCM 加密导出 + 解密加载 | 6h   | —        | 格式修复                    |
| TCP MotionServer (Slot 寻址)         | 20h  | —        | .mtn 完成                   |
| 影院视频播放器核心                   | 16h  | —        | LibVlcVideoPlayerService ✅ |
| 端到端联调                           | 4h   | —        | 以上全部                    |
...
| 任务                                   | 工时 | 前置依赖                 |
| -------------------------------------- | ---- | ------------------------ |
| IOStudio C# 播放路径替换为 C++ Wrapper | 4h   | C# Wrapper ✅ + 格式修复 |
| 核心引擎单元测试补齐                   | 8h   | 无                       |
| Avalonia 版本统一 + net8.0 升级评估    | 4h   | 无                       |

**节点四 (5/16~5/31)：**

| 任务                                      | 工时 |
| ----------------------------------------- | ---- |
| 效果预设系统 (预设库 + 拖拽 + 自动关键帧) | 16h  |
| 性能优化 (>1000 关键帧 >30fps)            | 3h   |
| 全系统联调                                | 8h   |
| Bug 回归修复                              | 4h   |
...
```
格式修复 (立即) → .mtn 加密 → TCP MotionServer → 播放器 → 联调    ← 节点二
                                    ↓
                      C# 播放路径统一 → 单元测试补齐                 ← 节点三
                                    ↓
                      效果预设 → 系统联调                             ← 节点四
                                    ↓
                      LicHper → 打包 → 文档 → 验收                   ← 节点五
```

### 5.5 关键依赖

```
格式修复 ──→ .mtn 加密 ──→ TCP MotionServer ──→ 端到端联调
C++ MotionPlayer ✅ ──→ C# 播放路径统一 ──→ 编辑器预览与运行时一致
```
...
| 日期 | 检查内容                                          |
| ---- | ------------------------------------------------- |
| 4/15 | 编辑器核心可用 + .motion 格式兼容修复完成         |
| 4/30 | 编辑器+TCP 端到端打通 + 影院播放器核心            |
| 5/15 | C++ MotionPlayer 驱动设备 + C# 播放路径统一       |
| 5/31 | 全系统联调通过 + 效果预设可用                     |
| 6/15 | 安装包初版 + 文档初稿                             |
| 6/30 | 产品验收：全部交付物就绪                          |
...
| ID  | 风险                                                         | 概率   | 影响    | 缓解措施                                 |
| --- | ------------------------------------------------------------ | ------ | ------- | ---------------------------------------- |
| R1  | ~~.motion 格式不兼容~~ — ✅ 已修复                           | ~~已确认~~ | ~~阻断~~ | 已修复 (§ 3.1)                           |
| R2  | **双引擎行为差异** — 编辑器预览与运行时播放结果不一致        | 高     | 🔴      | 统一播放路径 (§ 3.2)                     |
| R3  | **节点二工期紧张** — .mtn+TCP+播放器需短期内完成             | 高     | 🟡      | 严格按关键路径串行，不并行冒险           |
| R4  | ~~Avalonia NuGet 版本不一致~~ — ✅ 已修复                    | ~~中~~   | ~~🟡~~  | ItemsRepeater 包已移除                   |
| R5  | ~~net6.0 EOL~~ — ✅ 已升级至 net8.0                          | ~~中~~   | ~~🟡~~  | 已升级 + 19 测试通过                     |
| R6  | **核心引擎零测试覆盖** — 回归风险                            | 中     | 🟡      | 节点三补齐 8h 单元测试                   |
| R7  | **TCP 黏包/半包**                                            | 低     | 🟡      | 4 字节长度前缀 + ReadExactAsync          |
| R8  | **多 Slot 通道冲突** — 多文件同写一设备                      | 中     | 🟡      | ChannelMixer 仲裁 (默认 Priority)        |
| R9  | **AES 密钥被逆向** — .mtn 保护失效                           | 中     | 🟡      | 多层防护: IL混淆 + 密钥分散 + C++ native |
| R10 | **SkiaSharp 曲线编辑性能** — >500 关键帧卡顿                 | 中     | 🟡      | LOD 渲染 + SKPath 批量绘制               |
| R11 | **IODevice.dll SetDO 高频延迟**                              | 低     | 🟡      | 批量写入 + 降频至 30fps                  |
