...
> **版本：** v23 (v3.0 演进字段补档: idle/override/neutral/phase-group)
> **最后更新：** 2026-09-02  
...
> **源码位置：** `IODevice/Source/Public/MotionPlayer.h` + `IODevice/Source/Private/MotionPlayer.cpp` (~1180 行, 含 idle/overlay/phase-group 演进; 2026-09-01 同步)  
> **状态：** ✅ 已实现，构建验证通过  
...
> **插值类型兼容 (2026-08-31 修复)：** C++ `InterpolateKeyframes` 原先仅识别 `"step"` / `"bezier"` / `"ease"`，对 C# 编辑器写出的 `"ease_in_out"` 会落入 `default: linear` 分支，导致**编辑器预览 (C# 识 ease_in_out) 与运行时实播 (C++ 退化为 linear) 的隐性行为不一致**。已补识别 `"ease_in_out"` 等价 `"ease"` (smoothstep)。涉及 `IODevice/IODevice/Source/Private/MotionPlayer.cpp`。**约束：** `.motion` 文件插值字面量统一用 `linear | bezier | step | ease_in_out`；C++ 运行时须对 `ease_in_out` 与 `ease` 同义处理，保证"所见即所播"。

### 5.4 v3.0 演进字段 (2026-09-02 补档)

> 以下字段为 v1.0 基础格式之上已上线并写入生产 `.motion` 文件的演进字段，C# / C++ 均已支持（C# `JsonPropertyName` 与 C++ `value()` 带默认值解析）。

| 层级         | 字段                  | 类型   | 说明                                                                                                  |
| ------------ | --------------------- | ------ | ----------------------------------------------------------------------------------------------------- |
| root         | `version`             | string | 格式版本号 (`"3.0"`)                                                                                  |
| root         | `name`                | string | 项目/影片名称                                                                                          |
| root         | `fps`                 | number | 帧率 (编辑器 UI 对齐用)                                                                               |
| track        | `id`                  | string | 轨道唯一 ID (8 位 hex)                                                                                |
| track        | `label`               | string | 显示名称 (如 "俯仰")                                                                                  |
| track        | `color`               | string | 轨道颜色 (hex)                                                                                        |
| track        | `value_type`          | string | `"float"` 连续值 / `"bool"` 开关值                                                                    |
| track        | `neutral_value`       | number | 无覆盖时中性值 (bool=0 / float=0.5 缺省)                                                              |
| track        | `role`                | string | 逻辑角色标签 (如 `heave`/`pitch`/`roll`/`impact`/`stop`)，供预设按角色匹配                            |
| track        | `group`               | string | 轨道分组名                                                                                            |
| track        | `show_in_curve`       | bool   | 是否在曲线视图显示 (默认为 true)                                                                      |
| track        | `solo`/`locked`       | bool   | 独奏 / 锁定                                                                                           |
| track        | `idle` (object)       | object | **待机循环 (gap 填充)**：`enabled`/`period_ms`/`blend_ms`/`phase_mode`/`phase_offset_ms`/`group_id`/`keyframes[]` |
| idle         | `phase_mode`          | string | `"continuous"` 跨空窗连贯 / `"restart"` 每 gap 从头                                                   |
| idle         | `phase_offset_ms`     | number | 相位偏移 (同组多轨错相编排)                                                                          |
| idle         | `group_id`            | string | 节奏组标识 (同组 continuous 共享全局时钟)                                                              |
| track        | `override_keyframes[]` | array | **Overlay 覆盖关键帧 (绝对时间点)**：两点插值、首前/末后回落 idle、单点=脉冲                          |
| clip         | `id`                  | string | 片段唯一 ID                                                                                          |
| clip         | `action_instance_id`  | string | 生成此 clip 的动作实例 ID (空=手工 clip)                                                              |
| clip         | `source_definition_id` / `source_role` / `source_action_name` | string | 动作来源追溯元数据 |
| root         | `action_instances[]`  | array  | 影片级动作实例 (跨轨关联)                                                                             |
| root         | `role_track_bindings` | object | 逻辑角色 → 轨道 ID 绑定                                                                               |
| root         | `embedded_presets[]`  | array  | 项目使用过的预设快照 (外部库变化仍可重建)                                                             |
| root         | `groups[]` / `events[]` / `markers[]` | array | 分组 / 事件 / 标记 (含 `event_lanes`/`marker_lanes`)                             |

> **求值优先级 (C++ EvaluateTrack 三态)：** `clip 覆盖 > override_keyframes > idle 循环 > neutral_value`；`blend_ms` 与相邻 clip 交叉淡化 (idle→clip 淡入 / clip→idle 淡出)；bool 轨输出强制量化 0/1。
...
### 10.13 Cinema Workflow Profile 可扩展流程编排（2026-05-23 追加）

当前 `UNIPlayer/Assets/StreamingAssets/Configs/Demo/cinema-profile.json` 已具备 `workflow.prepare[] / workflow.finish[]` 的步骤数组，但上层消费仍存在两处硬编码：

- `UNIPlayer/Assets/StreamingAssets/Configs/Demo/影院.xml` 只把 workflow step 当成“设备命令队列”，尚未显式表达人工确认、自动推进、可选步骤、跳过原因等流程语义。
- `CinemaControl/src/stores/prepare.ts` 与 `CinemaControl/src/pages/PreparePage.vue` 仍把“压杆 -> 确认 -> 开门 -> 车前进 -> 播放 -> 收尾”写成固定状态机与固定 8 步 UI。

这会导致“极简影院流程（选片即可确认播放）”与“复杂动感影院流程（压杆/门/车/复位）”无法在同一套播控平台内通过配置切换，只能继续复制页面逻辑或追加 if 分支。

#### 10.13.1 设计目标

- 同一套协议同时支持“极简流程”和“复杂流程”。
- `CinemaControl` 不再预设“必须有压杆/舱门/车体”，只消费 profile 暴露出来的流程语义。
- `UNIPlayer` 继续作为运行时编排器，但只执行“当前 profile 定义过的步骤”，不把影院业务写死在 XML 分支里。
- 设备能力缺失、流程裁剪、场馆定制都通过 profile 完成，不新增一套协议。

#### 10.13.2 分层原则

```text
ProfileDefinition
    ├── Capabilities      // 站点具备什么能力: restraint/gate/carrier/media/motion
    ├── WorkflowTemplates // prepare / playback / finish 的步骤定义
    ├── UIHints           // 页面要显示哪些卡片、按钮、提示、步骤名
    └── Policies          // 自动返回待机、失败策略、超时策略、是否允许手动跳过

WorkflowRuntime
    ├── WorkflowPlan      // 当前 phase 展开的实际步骤清单
    ├── WorkflowStepState // pending/running/waiting/completed/skipped/failed
    └── Snapshot          // 给 CinemaControl 的统一快照
...
Simulator / Real Device Plugin
    ├── Capability Provider
    ├── Device State Emulator / Adapter
    ├── Feedback Publisher
    └── Optional Local Debug UI
```

关键边界：

- `capability` 表示系统能做什么，例如 `safety.restraint`、`access.gate`、`carrier.position`。
- `workflow step` 表示在当前场馆/当前 phase 下要不要做这一步。
- `ui hint` 表示前端如何解释和呈现这一步，而不是前端自己猜流程。
- `orchestrator` 只能有一个。当前最合适的是 `UNIPlayer`，因为它同时掌握媒体播放状态、workflow 定时、快照推送和设备通道绑定。

#### 10.13.2.1 三端统一事实源

要兼容简单流程与复杂流程，三端必须共享同一份“流程事实”，否则会重新出现各端各写一套状态机的问题。建议按以下事实源划分：

- `ProfileDefinition`：静态事实源。定义站点能力、步骤模板、UI 提示、策略开关。
- `WorkflowSnapshot`：运行时事实源。定义当前 phase、当前步骤、步骤状态、可执行动作。
- `CapabilityFeedback`：设备事实源。定义压杆、舱门、小车等真实或仿真的反馈状态。

对应归属：

- `ProfileDefinition` 由站点配置提供，运行时由 `UNIPlayer` 加载并向外发布摘要。
- `WorkflowSnapshot` 只能由 `UNIPlayer` 生成，因为只有它同时知道“流程推进”和“媒体状态”。
- `CapabilityFeedback` 由 `Simulator` 或真实设备插件产生，`UNIPlayer` 负责汇总入 snapshot。

这意味着：

- `CinemaControl` 不再推导流程，只消费 `WorkflowSnapshot`。
- `Simulator` 不再持有影院业务流程，只暴露能力、执行命令、推反馈。
- `UNIPlayer` 不再把所有影院流程硬编码在 XML 分支里，而是消费 profile 生成 runtime plan。

#### 10.13.3 Step 模型升级

现有 step 结构只有 `id/label/capability/command/target/required/timeoutMs`，不足以表达人工确认与流程裁剪。建议向后兼容扩展为：

```json
{
  "id": "awaitOperatorConfirm",
  "label": "确认播放",
  "phase": "prepare",
  "kind": "manual",
  "capability": null,
  "command": null,
  "guard": {
    "allOf": [{ "fact": "session.selectedFilm.present", "equals": true }]
  },
  "completion": {
    "type": "clientCommand",
    "command": "system.workflow.step.complete"
  },
  "ui": {
    "visible": true,
...
- `kind`: `manual | command | media | delay | derived`
- `guard`: 步骤是否可进入，不满足时保持 `blocked` 或直接 `skipped`
- `completion`: 该步骤靠什么完成，支持设备反馈、客户端确认、媒体状态变化、延时完成
- `ui`: 是否显示、显示文本、按钮样式、是否需要突出提示
- `skippable` / `skipReason`: profile 裁剪时能说明“为什么没这一步”

这样极简影院可以只有：

```text
prepare: selectFilm -> confirmStart -> loadMedia -> startMedia
finish:  playbackCompleted -> enterIdle
```

复杂动感影院则可以是：

```text
prepare: lockRestraint -> confirmStart -> openGate -> moveCarrier -> loadMedia -> startMedia
finish:  playbackCompleted -> openGate -> moveCarrierBack -> releaseRestraint -> closeGate -> enterIdle
```

两者共享同一套 step runtime，只是 profile 展开的步骤不同。

#### 10.13.4 Runtime 规则

`UNIPlayer` 中的 workflow runtime 应遵循以下状态模型：

```text
pending -> running -> waiting_feedback -> completed
                                                 |                |
                                                 |                -> skipped
                                                 -> failed
```
...
- `command` 步骤：由 `DispatchCinemaWorkflowCurrentStep()` 发出设备命令，再由反馈事件完成。
- `manual` 步骤：运行时不直接发设备命令，只把当前步骤暴露给 `CinemaControl`；由前端发 `system.workflow.step.complete(stepId)` 完成。
- `media` 步骤：由 `LoadFilmVideo()` / `StartRequestedMedia()` 等媒体动作完成，并以 `media.playback.*` 事件驱动收敛。
- `delay` 步骤：用于“开门后等待 2 秒”“播放完成后显示完成页 20 秒再回待机”这类站点策略。
- `derived` 步骤：纯 UI/叙事步骤，不发命令，只用于表达“等待乘客就位”“影片放映中”等业务语义。

#### 10.13.5 前端边界重构

`CinemaControl` 前端应从“本地 FSM”改为“消费 runtime snapshot 的 phase renderer”：

- `prepare.ts` 不再声明固定 `WorkflowStep` 联合类型，也不本地串行调用 `setGate()` / `setCarrier()` / `loadFilm()`。
- `PreparePage.vue` 不再假设“压杆未锁定就不能确认播放”，而是根据当前 profile 暴露的 `currentStep`, `availableActions`, `visibleSteps` 渲染。
- 本地 store 只负责三类事：
  - 拉取 profile summary / workflow snapshot
  - 映射 UI 状态（按钮是否可点、文案、错误）
  - 发送用户动作（确认、跳过、重试、复位）

前端因此从“流程编排者”降级为“流程消费者”，避免浏览器和 UNIPlayer 双份状态机长期漂移。

#### 10.13.6 推荐快照模型

建议 `system.workflow.snapshot` 在现有基础上补充：

```json
{
  "profileId": "demo.motion_carrier_cinema",
  "phase": "prepare",
  "state": "running",
  "currentStepId": "awaitOperatorConfirm",
  "steps": [
    {
      "id": "awaitOperatorConfirm",
      "label": "确认播放",
  ...
  "availableActions": [
    {
      "id": "complete-current-step",
      "stepId": "awaitOperatorConfirm",
      "label": "确认播放",
      "style": "primary"
    }
  ]
...
这样前端只需渲染 snapshot，不需要再根据 `gateState/restraintState/carrierState` 自己推导整条业务流程。

#### 10.13.6.1 UNIPlayer 的职责收口

对于三端协同，`UNIPlayer` 不只是“播放器”，而应成为影院运行时控制面的唯一编排器。建议职责收口为：

- 加载 `cinema-profile.json`，生成 `prepare / finish` 的运行时步骤计划。
- 根据 step `kind` 执行命令、等待反馈、推进超时、生成失败信息。
- 把 `media.playback.*`、`safety.*`、`access.*`、`carrier.*` 合并为统一 `WorkflowSnapshot` 与 `SystemSnapshot`。
- 作为 `Simulator` 与 `CinemaControl` 的协议边界，将底层插件能力差异吸收到 `CapabilityAdapter` 中。

因此后续 `影院.xml` 的重构重点不是把更多业务写进 XML，而是让 XML 只承担：

- channel 绑定
- workflow dispatch hook
- feedback to snapshot 汇总
- profile step 执行入口

具体影院流程语义应逐步迁移到 profile schema + runtime runner，而不是继续堆叠 `if(_workflow_command EQ ...)`。

#### 10.13.6.2 Simulator 的职责收口

`Simulator` 在这套架构里不是第二个 workflow engine，而是“能力提供者 + 状态反馈源 + 调试视图”。

依据当前实现，Simulator 已经具备：

- `_capabilities`：返回插件能力声明 JSON
- `simulator.command`：执行开门、压杆、小车等文本命令
- Web API / WebSocket：输出 `capabilities` 与 `snapshot`

因此建议明确边界：

- `Simulator` 负责声明“我支持哪些 capability / channel / topic / timing 参数”。
- `Simulator` 负责把命令映射为状态机推进，并产生 `_event` / snapshot 反馈。
- `Simulator` 不负责决定“这一步该不该做”“什么时候开始 finish phase”“是否允许确认播放”。
- `Simulator` 本地 Web UI 与内置 UI 也应消费同一份 capability + snapshot 语义，而不是重新内嵌一套影院业务流程。

这条边界非常关键：

- 如果把影院流程继续塞进 Simulator，就会导致不同 Simulator 模板各自长出业务分支。
- 如果把业务步骤只写在 CinemaControl，就会导致换前端或脱离前端时流程失真。
- 只有把流程编排固定在 `UNIPlayer`，Simulator 才能同时服务联调、真机替身、Web 调试页三类场景。

#### 10.13.6.3 Profile 的三端消费方式

同一份 profile 在三端中的消费方式应不同：

| 组件            | 消费内容                                                                     | 不应承担               |
| --------------- | ---------------------------------------------------------------------------- | ---------------------- |
| `CinemaControl` | `profile.summary`, `workflow.snapshot`, `ui hints`, `availableActions`       | 本地编排设备动作       |
| `UNIPlayer`     | 全量 profile：capabilities, workflow templates, policies, simulation mapping | 设备插件内部状态机实现 |
| `Simulator`     | capability subset, simulation timings, command mapping, debug labels         | 影院业务 phase 决策    |

建议把现有 profile 拆成逻辑上四块：

- `profile`: 站点身份与展示信息
- `capabilities`: 站点能力和设备约束
- `workflow`: phase 模板和步骤语义
- `simulation`: 仿真时序、命令映射、调试展示参数

这样简单影院与复杂影院都能复用同一 schema，只是在 `workflow` 和 `simulation` 上裁剪。

#### 10.13.6.4 Simulator 兼容真实设备的抽象方式

为了同时兼容真实设备与仿真器，`WorkflowRuntime` 不应直接依赖 `simulator.command` 这类调试命令，而应只依赖 capability command：

```text
Workflow Step
        -> Capability Command
        -> Runtime Adapter
                -> Simulator command / Plugin RPC / Real Device DO write
```

例如：

- `carrier.position.set target=presentation`
  - 在 Simulator 中可映射为 `simulator.command = carrier.presentation`
  - 在真实设备中可映射为 `SetDO(CarrierToPresentation)` 或厂商插件 RPC

这样 profile 里写的是影院语义，不是某个仿真器的私有命令。

#### 10.13.6.5 Profile 存放策略（考虑打包后）

`profile` 应继续跟随影院配置包放在 `StreamingAssets/Configs/<PackageName>/` 目录下，而不是散落到代码目录或插件目录。原因：

- `影院.xml` 当前就是从 `StreamingAssets/Configs/...` 加载，`cinemaProfile="cinema-profile.json"` 默认按 XML 所在目录解析。
- Windows 打包后，`StreamingAssets/Configs/...` 仍然是可见目录，便于整包交付、替换和现场运维。
- Android 打包后，`StreamingAssets` 在 APK 内是只读资源，适合作为默认 profile 的只读来源；如果需要现场覆盖，再单独引入 runtime override 目录。

推荐采用“两层存放”策略：

```text
Layer 1: Package Default (只读基线)
    StreamingAssets/Configs/<Package>/影院.xml
    StreamingAssets/Configs/<Package>/cinema-profile.json

Layer 2: Runtime Override (可写覆盖)
    Windows:   <AppDir>/Profiles/<Package>/cinema-profile.override.json
    Android:   <filesDir>/IOToolkit/Profiles/<Package>/cinema-profile.override.json
```

加载优先级建议：

1. `system.profile.select` 显式指定的 absolute / relative 路径
2. runtime override 目录中的站点覆盖 profile
3. `影院.xml` 同目录 `cinema-profile.json`
4. fallback `Configs/Demo/cinema-profile.json`

这样可以同时满足：

- 开发态：直接改 `StreamingAssets/Configs/<Package>/cinema-profile.json`
- Windows 交付：整包复制 `Configs/<Package>` 即可切换影院
- Android 交付：APK 内带默认 profile，必要时从后台、U 盘同步或运维工具下发 override 到 app-private 目录

边界建议：

- 默认 profile 必须跟影院配置包一起发布，确保“换一个包就换一套影院行为”。
- override profile 只用于现场微调、A/B 测试、应急兼容，不应成为长期主配置来源。
- `影院.xml` 与 `profile` 应位于同一个 package 目录，避免“XML 指向别处 profile”造成交付混乱。
- `IODevice.xml` 仍放在 IOToolkit runtime/config 体系，不并入 profile。
...
- 同一大类场馆如果只差少量流程参数，应优先共用一份 `影院.xml`，只替换不同 package 的 `cinema-profile.json`。
- 只有协议接线、页面结构或设备胶水差异很大时，才拆新的 `影院.xml` package。

#### 10.13.6.7 常规场景兼容矩阵

下面是实际常见场馆/设备流程，以及当前 `capability + workflow + snapshot` 方案的兼容性判断：

| 场景                       | 常见流程                                           | 当前方案兼容性 | 备注                                             |
| -------------------------- | -------------------------------------------------- | -------------- | ------------------------------------------------ |
| 普通影院（无动感）         | 选片 -> 确认播放 -> 播放 -> 完成 -> 回待机         | ✅ 直接兼容    | 只保留 `media.playback` + `manual confirm`       |
| 普通影院（带闸门/幕门）    | 选片 -> 确认 -> 开门/关门 -> 播放 -> 完成          | ✅ 直接兼容    | `access.gate` 可作为 optional capability         |
| 动感平台（压杆，无车）     | 选片 -> 压杆锁定 -> 确认 -> 播放 -> 释放压杆       | ✅ 直接兼容    | `safety.restraint` + `media.playback`            |
| 动感平台（压杆 + 车 + 门） | 压杆 -> 确认 -> 门 -> 车 -> 播放 -> 门/车/压杆回位 | ✅ 直接兼容    | 当前 Demo 就是这个主场景                         |
| DarkRide / 轨道车项目      | 发车 -> 分段场景触发 -> 到站 -> 开门/放客          | ⚠️ 部分兼容    | 需要增加 `route/segment/station` 能力与 phase    |
| 多特效影院                 | 选片 -> 播放 -> 风/水/雾/灯联动                    | ✅ 直接兼容    | 特效更像 `motion.effect` / `.motion` 驱动        |
| 带播前安全确认的影院       | 选片 -> 检票/安全确认 -> 压杆/门 -> 播放           | ✅ 直接兼容    | 用 `manual` step 表达人工确认                    |
| 播后保洁/清场流程          | 完成 -> 开门 -> 等待清场 -> 回待机                 | ✅ 直接兼容    | 需要 `delay/manual complete` 步骤                |
| 维护模式 / 手动跳步        | 维护员手动执行某步或跳过某步                       | ⚠️ 部分兼容    | 需要 `availableActions.skip/retry/forceComplete` |
| 紧急停止后恢复             | 紧停 -> 保持现场 -> 人工确认 -> 从某步恢复         | ⚠️ 部分兼容    | 需要 snapshot 恢复点与 step resume 语义          |
| 多厅联动 / 中央排片        | 多个 UNIPlayer 被同一调度系统编排                  | ⚠️ 部分兼容    | 单厅模型可复用，但缺跨实例编排层                 |

#### 10.13.6.8 当前方案已经覆盖的能力

当前方案已经天然适合以下类型的问题：

- 线性 prepare / finish 流程
- capability optional / required 裁剪
- 同一场馆存在“极简流程”和“完整流程”两个 profile 变体
- 人工确认、自动命令、媒体步骤的统一串接
- Simulator 与真实设备共用一份业务 profile

#### 10.13.6.9 当前方案尚未完全覆盖的能力

当前如果要覆盖更复杂影院，还需要继续补这几类语义：

- `branch`：根据票种、设备状态、场次类型走不同分支
- `parallel`：例如门和灯光、广播和准备动画可并行推进
- `loop/retry`：例如门未到位时重试、等待人工处理后继续
- `resume policy`：紧停后从哪一步恢复，是回退到 phase 起点还是从当前 step 恢复
- `role-based action`：操作员、维修员、中控系统的动作权限不同
- `multi-instance orchestration`：多厅、多车、多舱协同

这意味着：当前架构**足够支撑常规影院和绝大多数动感平台项目**，但如果目标扩展到 dark ride、联动场馆或复杂恢复策略，需要把 step 模型再提升为支持 branch / parallel / resume 的更完整 workflow DSL。

#### 10.13.6.10 全链路编排方案的文档边界

自本阶段起，“影院流程编排入口、`CinemaControl` 管理页、`authoring.json`、配置包导出与 Android 下发”不再视为 `IOStudio` 局部设计问题，而归类为 `CinemaControl ↔ UNIPlayer ↔ Simulator` 的全链路方案。

因此：

- 全链路架构结论统一维护在 [docs/播控全链路架构文档.md](o:/DevModules/IODevice/docs/播控全链路架构文档.md)。
- 分阶段实施项统一维护在 [docs/动感平台播控链路能力化迁移实施计划.md](o:/DevModules/IODevice/docs/动感平台播控链路能力化迁移实施计划.md)。
- 本文档只保留 `IOStudio` 在该方案中的边界：继续负责 `.motion` 编辑、动作文件产物与底层 ABI 约束，不再承载 `CinemaControl` 编排器主方案描述。

#### 10.13.7 渐进迁移策略

为避免一次性重写，建议分三步迁移：

1. 保留现有 `影院.xml` workflow dispatch，不改协议名；先把 profile step 扩展出 `kind/guard/completion/ui` 字段，并让 snapshot 带出 `visibleSteps/currentStep/availableActions`。
2. `UNIPlayer` 增加 capability command 到 runtime adapter 的映射层，避免 workflow 直接写死 `simulator.command` 细节。
3. `CinemaControl` 改成“优先消费 workflow snapshot；缺字段时退回旧逻辑”，完成 UI 解耦。
4. `Simulator` Web / 内置 UI 改为消费统一 capability + snapshot 语义，不再单独描述影院业务 phase。
5. 等前端与调试 UI 都切到 snapshot 后，再删除 `prepare.ts` 内部固定状态机和串行设备控制代码。

#### 10.13.8 结论

结论不是“再增加几个可选开关”，而是把播控流程正式拆成三层：

- `Capability`：设备能做什么
- `Workflow Profile`：当前影院想按什么步骤运行
- `UI Projection`：当前步骤在页面上怎么呈现

只有这样，简单影院和复杂影院才能共享一套播控协议与运行时，而不是共享一堆越来越难维护的条件分支。
